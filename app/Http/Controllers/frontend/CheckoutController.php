...
class CheckoutController extends Controller
{
    public function checkoutPage()
    {
        ...
        return view('frontend.page.checkout', compact(
            'carts',
            'total_price',
            'districts'
        ));
    ...
    public function placeOrder(OrderStoreRequest $request)
    {
        ...
        $order = Order::create([
            ...
            'sub_total' => Session::get('coupne')['cart_total'] ?? round(Cart::subtotalFloat()),
            'discount_amount' => Session::get('coupne')['discount'] ?? 0,
            'coupon_name' => Session::get('coupne')['name'] ?? '0',
            'total' => Session::get('coupne')['balance'] ?? round(Cart::subtotalFloat()),
        ]);
        // dd($order);
        ...
        return redirect()->route('shopping.cart.page');
